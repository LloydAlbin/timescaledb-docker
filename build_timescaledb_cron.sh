...
for PGVERSION in ${PGVERSION_Array[@]}; do
    ...
    ~/timescaledb-docker/build_timescaledb.sh -v -v -v -V -pgv $PGVERSION --org $DOCKER_ORG --pg_name $DOCKER_IMAGE_NAME_POSTGRES --ts_name $DOCKER_IMAGE_NAME_TIMESCALE --push --clean --override_exit
